...
/** §GLITCH uses OTC curve; other tokens use Jupiter from /api/trade/prices. */
export function mergeOtcGlitchPrice(
  prices: TradePrices,
  otcPriceUsd: number | null | undefined,
): TradePrices {
  if (otcPriceUsd == null || !Number.isFinite(otcPriceUsd)) return prices;
  return { ...prices, GLITCH: otcPriceUsd };
}
